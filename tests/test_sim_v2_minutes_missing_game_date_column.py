from __future__ import annotations

import json
from pathlib import Path

import pandas as pd

from scripts.sim_v2.generate_worlds_fpts_v2 import main as generate_worlds_main


def test_sim_v2_accepts_minutes_parquet_missing_game_date_column(tmp_path: Path) -> None:
    game_date = "2025-01-03"

    profiles_path = tmp_path / "sim_profiles.json"
    profiles_path.write_text(
        json.dumps(
            {
                "profiles": {
                    "test_missing_game_date": {
                        "mean_source": "rates",
                        "minutes_source": "minutes_v1",
                        "rates_source": "rates_v1_live",
                        "worlds": {"n_worlds": 5, "batch_size": 5},
                        "noise": {"epsilon_dist": "normal", "nu": 5, "k_default": 0.0},
                        "rates_noise": {"enabled": False},
                        "minutes_noise": {"enabled": False, "sigma_min": 0.0},
                        "team_factor_sigma": 0.0,
                        "team_factor_gamma": 1.0,
                        "enforce_team_240": False,
                        "use_play_prob_masking": False,
                        "seed": 1337,
                    }
                }
            },
            indent=2,
        ),
        encoding="utf-8",
    )

    # Minutes: intentionally omit `game_date` to mimic RMH v1 outputs.
    minutes_day = tmp_path / "gold" / "projections_minutes_v1" / f"game_date={game_date}"
    minutes_day.mkdir(parents=True, exist_ok=True)
    run_id = "test"
    (minutes_day / "latest_run.json").write_text(json.dumps({"run_id": run_id}), encoding="utf-8")
    minutes_run = minutes_day / f"run={run_id}"
    minutes_run.mkdir(parents=True, exist_ok=True)
    pd.DataFrame(
        [
            {
                "tip_ts": f"{game_date}T00:00:00Z",
                "game_id": 1,
                "team_id": 10,
                "player_id": 100,
                "player_name": "A",
                "status": "available",
                "starter_flag": 1,
                "is_projected_starter": 1,
                "play_prob": 1.0,
                "minutes_p10": 28.0,
                "minutes_p50": 32.0,
                "minutes_p90": 36.0,
            },
            {
                "tip_ts": f"{game_date}T00:00:00Z",
                "game_id": 1,
                "team_id": 20,
                "player_id": 200,
                "player_name": "B",
                "status": "questionable",
                "starter_flag": 0,
                "is_projected_starter": 0,
                "play_prob": 0.4,
                "minutes_p10": 14.0,
                "minutes_p50": 18.0,
                "minutes_p90": 22.0,
            },
        ]
    ).to_parquet(minutes_run / "minutes.parquet", index=False)

    # Rates: minimal columns needed for compute_dk_fpts with efficiency.
    rates_day = tmp_path / "gold" / "rates_v1_live" / game_date
    rates_day.mkdir(parents=True, exist_ok=True)
    pd.DataFrame(
        [
            {
                "game_date": game_date,
                "game_id": 1,
                "team_id": 10,
                "player_id": 100,
                "fga2_per_min": 0.6,
                "fga3_per_min": 0.3,
                "fta_per_min": 0.2,
                "ast_per_min": 0.1,
                "tov_per_min": 0.05,
                "oreb_per_min": 0.05,
                "dreb_per_min": 0.15,
                "stl_per_min": 0.03,
                "blk_per_min": 0.02,
                "fg2_pct": 0.52,
                "fg3_pct": 0.36,
                "ft_pct": 0.78,
            },
            {
                "game_date": game_date,
                "game_id": 1,
                "team_id": 20,
                "player_id": 200,
                "fga2_per_min": 0.6,
                "fga3_per_min": 0.3,
                "fta_per_min": 0.2,
                "ast_per_min": 0.1,
                "tov_per_min": 0.05,
                "oreb_per_min": 0.05,
                "dreb_per_min": 0.15,
                "stl_per_min": 0.03,
                "blk_per_min": 0.02,
                "fg2_pct": 0.52,
                "fg3_pct": 0.36,
                "ft_pct": 0.78,
            },
        ]
    ).to_parquet(rates_day / "rates.parquet", index=False)

    output_root = tmp_path / "out"
    generate_worlds_main(
        start_date=game_date,
        end_date=game_date,
        n_worlds=5,
        profile="test_missing_game_date",
        data_root=tmp_path,
        profiles_path=profiles_path,
        output_root=output_root,
        sim_run_id=None,
        use_rates_noise=False,
        rates_noise_split=None,
        team_sigma_scale=None,
        player_sigma_scale=None,
        rates_run_id=None,
        minutes_run_id=None,
        use_minutes_noise=False,
        minutes_noise_run_id=None,
        minutes_sigma_min=None,
        seed=1337,
        min_play_prob=None,
        team_factor_sigma=None,
        team_factor_gamma=None,
        use_efficiency_scoring=True,
    )

    proj_path = output_root / f"game_date={game_date}" / "projections.parquet"
    df = pd.read_parquet(proj_path)
    assert "game_date" in df.columns
    assert pd.to_datetime(df["game_date"], errors="coerce").dt.date.nunique() == 1

