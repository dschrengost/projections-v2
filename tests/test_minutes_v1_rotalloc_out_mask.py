...
def test_rotalloc_respects_post_scoring_out_mask(tmp_path: Path) -> None:
    ...
    env_override = {
        "PROJECTIONS_DATA_ROOT": str(data_root),
        "PROJECTIONS_ROTALLOC_FAIL_HARD": "",
...
def test_occupancy_sparse_v0_respects_out_mask_and_team_total(tmp_path: Path) -> None:
    data_root, features_run_dir = _write_live_features(tmp_path)
    ...
    assert not out_rows.empty
    assert (pd.to_numeric(out_rows["minutes_p50"], errors="coerce").fillna(0.0) == 0.0).all()
    assert (pd.to_numeric(out_rows["play_prob"], errors="coerce").fillna(0.0) == 0.0).all()

    active = status_upper != "OUT"
    totals = (
        frame.loc[active]
        .assign(minutes_p50=pd.to_numeric(frame.loc[active, "minutes_p50"], errors="coerce").fillna(0.0))
        .groupby(["game_id", "team_id"])["minutes_p50"]
    ...
    alloc_mode_values = set(frame["minutes_alloc_mode"].dropna().astype(str).unique().tolist())
    assert alloc_mode_values == {"occupancy_sparse_v0"}
